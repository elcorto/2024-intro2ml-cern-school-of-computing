...
\begin{tikzpicture}[shorten >=1pt,node distance=3cm,auto]
  ...
  \node[dashed,draw, rectangle, text width=1.25cm, align=right] (fixed) [left of=input, node distance=2cm] {$x=2$ $W=.5$ $\beta=1$};


   \node[ultra thick, draw, ellipse] (hidden) [right of=input] {$\sigma(W \cdot x + \beta)$};
   \node[ultra thick, draw, circle] (output) [right of=hidden] {Output $\hat{y}$};
   \node[ultra thick, draw, rectangle, minimum width=2.5cm] (loss) [right of=output] {$\mathcal{L}(y, \hat{y})$};

   \path[->] (input) edge node {W} (hidden)
   (hidden) edge (output)
   (output) edge (loss);
   \node[below=.5em of loss] {$= (y-\hat{y})^2$};
   \node (ytrue) [right of=loss, node distance=2.5cm] {$y = 1.5$};
